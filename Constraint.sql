...
-- NOT NULL 제약조건이 걸린 not_null_column에 값을 지정하지 않아서 실행되지 않는다
...
-- UNIQUE로 지정된 컬럼은 중복된 INSERT시에 중복된 값을 지정할 수 없음
INSERT INTO unique_table VALUES (1, 1);
INSERT INTO unique_table VALUES (2, 1);
-- UNIQUE로 지정된 컬럼은 UPDATE시에 동일한 값으로 변경할 수 없음
-- 실행중 중간에 실패해도 원래 값으로 되돌아감의 예시
UPDATE unique_table SET unique_column = 3;

-- NOT NULL + UNIQUE = 후보키
-- 후보키: 레코드에서 레코드를 식별하기 위한 컬럼 (중복 X, null X)
CREATE TABLE candidate_table (
	cnadidate_column INT NOT NULL UNIQUE,
    unique_column INT UNIQUE
);
...
-- PRIMARY KEY 제약조건은 INSERT시에 NOT NULL의 조건과 UNIQUE의 조건을 모두 만족해야함
...
-- FOREIGN KEY: 특정 컬럼을 다른 테이블 혹은 같은 테이블의 기본키 컬럼과 연결하는 역할
...
-- FOREIGN KEY 제약 조건이 걸린 컬럼에 INSERT 작업시
-- 참조하고 있는 테이블의 컬럼에 값이 존재하지 않으면 지정 할 수 없음
...
-- 특정 테이블을 참조하고있는 테이블이 존재한다면 테이블 구조를 삭제하지 못함
DROP TABLE primary_table;
-- 특정 테이블을 참조하고있는 테이블이 존재한다면 테이블 구조를 수정하지 못함
UPDATE primary_table 
SET primary_column = 3 
WHERE primary_column =1;

-- ON UPDATE / ON DELETE 옵션 (FORIEGN KEY의 옵션들)
-- ON UPDATE: 참조하고있는 테이블의 기본키가 변경되었을 때 동작
-- ON DELETE: 참조하고있는 테이블의 기본키가 삭제되었을 때 동작

-- CASCADE : 참조하고있는 테이블에서 데이터를 삭제하거나 
-- 수정했을 때, 참조하는 테이블에서도 삭제와 수정이 같이 일어남

-- SET NULL : 참조하고있는 테이블에서 데이터를 삭제하거나
-- 수정했을 때, 참조하는 테이블에 해당 데이터를 null로 지정
...
-- CHECK로 지정된 컬럼은 지정된 조건에 부합하지 않으면 INSERT 불가능
INSERT INTO check_table VALUES(2, '남자');
-- CHECK로 지정된 컬럼은 지정된 조건에 부합하지 않으면 UPDATE 불가능
UPDATE check_table SET check_column = '남자';

-- DEFAULT 제약조건: 컬럼에 데이터가 지정되지 않았을 때 사용할기본값 지정
